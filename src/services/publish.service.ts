...
export class PublishService extends Repository {
    ...
    public async reel(options: PostingReelOptions) {
        ...
        const form: MediaConfigureToReelOptions = {
            ...
            audience: 'default',
            is_clips_edited: '0',
            clips_share_preview_to_feed:
                typeof options.shareToFeed == 'boolean' ? (options.shareToFeed ? '1' : '0') : '1',
            is_shared_to_fb: typeof options.shareToFB == 'boolean' ? (options.shareToFB ? '1' : '0') : '0',
            like_and_view_counts_disabled:
                typeof options.likeAndViewCountDisable == 'boolean'
                    ? options.likeAndViewCountDisable
                        ? '1'
                        : '0'
                    : '0',
        ...
        if (options.music) {
            ...
            if (options.music.params)
                form.music_params = {
                    ...
                    ...(options.music.params.browseSessionId && {
                        browse_session_id: options.music.params.browseSessionId,
                    }),
                    ...(options.music.params.alacornSessionId && {
                        alacorn_session_id: options.music.params.alacornSessionId,
                    }),
                };
            if (options.music.metadata)
                form.clips_audio_metadata = {
                    ...
                    song: {
                        ...
                        audio_cluster_id:
                            options.music.metadata.song.audioClusterId ?? options.music.params.audioClusterId,
